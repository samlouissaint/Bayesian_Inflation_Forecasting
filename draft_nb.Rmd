...
model_components <- AddSemilocalLinearTrend(model_components, monthly_train$P)
model_components <- AddSeasonal(model_components, y = monthly_train$P, nseasons = 12)
fit <- bsts(monthly_train$P, model_components, niter = 2000)
plot(fit)
```

# Create Priors about economic indicators and Re-fit
```{r}
Pmu <- NormalPrior(2.5, 0.3)
Psd <- SdPrior(0.2,0.3)
```
...
model_components <- list()
model_components <- AddSemilocalLinearTrend(model_components, monthly_train$P, slope.mean.prior = Pmu, slope.sigma.prior = Psd)
model_components <- AddSeasonal(model_components, y = monthly_train$P, nseasons = 32, season.duration = 12)
fit2 <- bsts(P~ I, model_components, data = monthly_train, niter = 2000)
```


```{r}
plot(fit2, 'components')
pred1 <- predict(fit2, horizon = 12, newdata = Itest$DFF)
plot(pred1, ylim = c(-20, 20))
```
...
predictions = monthly_train %$%
  ...
  add_draws(plogis(predict(fit2, horizon = forecast_months, newdata = monthly_test$I[1:13])$distribution), value = ".prediction")
