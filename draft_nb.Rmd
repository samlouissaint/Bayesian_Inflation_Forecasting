...
I_m1_train <- rbind(I[1,], I) %>%
  ...
  rename(I_m1 = DFF)
I_m1_test <- I %>%
  filter(DATE >= as.POSIXlt('2021-06-01'), DATE < as.POSIXlt('2022-02-01')) %>%
  rename(I_m1 = DFF)
...
pred1 <- predict(model, horizon = 8, newdata = monthly_test)
plot(pred1)
```

```{r}
forecast_months = 8 # number of months forward to forecast
set.seed(123456)
y_max = .1
y_axis = list(
